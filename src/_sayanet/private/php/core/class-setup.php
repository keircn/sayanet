...
class Setup {
    ...
    private function add_app_metadata() {
        $this->set('NAME', 'sayanet');
        // support both new and legacy version constants
        $version = defined('SAYANET_VERSION') ? SAYANET_VERSION : (defined('H5AI_VERSION') ? H5AI_VERSION : '0.0.0');
        $this->set('VERSION', $version);
        $this->set('FILE_PREFIX', '_sayanet');
    }
    ...
    private function add_paths() {
        ...
        $this->set('SAYANET_HREF', Util::normalize_path(dirname(dirname($script_name)), true));
        $this->set('SAYANET_PATH', Util::normalize_path(dirname(dirname(dirname(dirname(__FILE__)))), false));
        // legacy aliases
        $this->store['H5AI_HREF'] = $this->store['SAYANET_HREF'];
        $this->store['H5AI_PATH'] = $this->store['SAYANET_PATH'];

        $root_path_override = getenv('SAYANET_ROOT_PATH');
        if ($root_path_override === false) {
            $root_path_override = getenv('H5AI_ROOT_PATH');
        }
        if ($root_path_override !== false && is_dir($root_path_override)) {
            $this->set('ROOT_PATH', Util::normalize_path($root_path_override, false));
        } else {
            $this->set('ROOT_PATH', Util::normalize_path(dirname($this->get('SAYANET_PATH')), false));
        }

        $this->set('ROOT_HREF', Util::normalize_path(dirname($this->get('SAYANET_HREF')), true));

        $this->set('PUBLIC_HREF', Util::normalize_path($this->get('SAYANET_HREF') . '/public/', true));
        $this->set('PUBLIC_PATH', Util::normalize_path($this->get('SAYANET_PATH') . '/public/', false));
        ...
        $this->set('PRIVATE_PATH', Util::normalize_path($this->get('SAYANET_PATH') . '/private', false));
